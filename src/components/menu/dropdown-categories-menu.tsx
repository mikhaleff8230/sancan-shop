    ...
    <div className="flex items-center justify-between px-4 py-3 border-b border-gray-200 bg-white flex-shrink-0">
      {mobileNav.level !== 'level1' ? (
        <button
          ...
          className="flex items-center justify-center w-10 h-10 -ml-2 text-gray-700 hover:text-brand transition-colors focus:outline-none"
          aria-label="Назад"
      ...
      <button
        className="flex items-center justify-center w-10 h-10 -mr-2 text-gray-400 hover:text-gray-700 focus:outline-none"
    ...
    <div className="relative flex-1 overflow-y-auto">
      ...
      <div
        className={`transition-transform duration-300 ease-in-out ${
          mobileNav.level === 'level1' ? 'translate-x-0' : '-translate-x-full'
        }`}
      >
        <div className="p-4">
          {categoriesData.map((cat) => {
            const hasSubcategories =
              cat.columns.some(col => col.sections.some(section => (section.items && section.items.length > 0) || (section.slug && section.slug !== '#'))) ||
            ...
            return (
              <button
                ...
                className="w-full flex items-center justify-between px-4 py-4 mb-2 bg-white border border-gray-200 rounded-lg hover:border-brand hover:bg-brand/5 active:bg-brand/10 transition-all duration-150 focus:outline-none"
                style={{ touchAction: 'manipulation', cursor: 'pointer' }}
              >
                <div className="flex items-center gap-3 flex-1 min-w-0">
                  <span className="text-2xl flex-shrink-0">{getCategoryIcon(cat.icon)}</span>
                  <span className="text-base font-semibold text-gray-800 text-left flex-1 truncate">{cat.name}</span>
                </div>
                {hasSubcategories && <ChevronRight className="w-5 h-5 text-gray-400 flex-shrink-0" />}
              </button>
            );
          })}
        </div>
      ...
      {mobileNav.categoryId && (() => {
        ...
        category.columns.forEach(col => col.sections.forEach(section => allSections.push(section)));

        return (
          <div
            className={`absolute inset-0 transition-transform duration-300 ease-in-out ${
              mobileNav.level === 'level2' ? 'translate-x-0 z-20' : mobileNav.level === 'level3' ? '-translate-x-full z-10' : 'translate-x-full z-0'
            }`}
          >
            <div className="p-4">
                ...
                allSections.map((section, idx) => {
                  ...
                  return (
                    <button
                      key={idx}
                      onClick={() =>
                        handleMobileSectionClick(section.title, section.slug || '#', hasSubmenu)
                      }
                      className="w-full flex items-center justify-between px-4 py-4 mb-2 bg-white border border-gray-200 rounded-lg hover:border-brand hover:bg-brand/5 active:bg-brand/10 transition-all duration-150 focus:outline-none"
                      style={{ touchAction: 'manipulation', cursor: 'pointer' }}
                    >
                      <span className="text-base font-semibold text-gray-800 text-left flex-1">{section.title}</span>
                      {hasSubmenu && <ChevronRight className="w-5 h-5 text-gray-400 flex-shrink-0" />}
                    </button>
                  );
                })
              ) : (
                <div className="text-center py-8 text-gray-400">Нет подкатегорий</div>
              )}
            </div>
          </div>
        );
      ...
      {mobileNav.categoryId && mobileNav.sectionSlug && (() => {
        ...
        if (!targetSection || !targetSection.items || targetSection.items.length === 0) return null;
