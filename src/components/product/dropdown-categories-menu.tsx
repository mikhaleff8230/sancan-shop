    ...
    >
      <div className="p-4">
        {allSections.map((section, idx) => {
          ...
          const hasSlug = typeof section.slug === 'string' && section.slug !== '#' && section.slug.trim() !== '';

          if (!hasItems && hasSlug) {
            // Секция без подкатегорий → ссылка
            return (
              <Link
                key={idx}
                prefetch={false}
                ...
                className="block w-full flex items-center justify-between px-4 py-4 mb-2 bg-white border border-gray-200 rounded-lg hover:border-brand hover:bg-brand/5 transition-all duration-150 focus:outline-none min-h-[56px]"
              >
                <span className="text-base font-semibold text-gray-800 flex-1">{section.title}</span>
              </Link>
            );
          }

          // Секция с подкатегориями → кнопка
          return (
            <button
              key={idx}
              type="button"
              onClick={() => {
                if (hasItems) {
                  ...
                  window.location.href = `/categories/${section.slug}`;
                }
              }}
              className="w-full flex items-center justify-between px-4 py-4 mb-2 bg-white border border-gray-200 rounded-lg hover:border-brand hover:bg-brand/5 transition-all duration-150 focus:outline-none min-h-[56px]"
            >
              <span className="text-base font-semibold text-gray-800 flex-1">{section.title}</span>
              {hasItems && <ChevronRight className="w-5 h-5 text-gray-400" />}
            </button>
          );
        })}
      </div>
...
{mobileNav.level === 'level3' && mobileNav.categoryId && mobileNav.sectionTitle && (() => {
  const category = categoriesData.find(c => c.id === mobileNav.categoryId);
  if (!category) return null;
  ...
  category.columns.forEach(col => {
    col.sections.forEach(section => {
      if (section.title === mobileNav.sectionTitle) targetSection = section;
    });
  });
  if (!targetSection || !Array.isArray(targetSection.items)) return null;

  return (
    <div
      className="absolute inset-0 overflow-y-auto scrollbar-hide transition-transform duration-300 ease-in-out bg-white translate-x-0 z-30"
      style={{ WebkitOverflowScrolling: 'touch' }}
    >
      <div className="p-4">
        {targetSection.items.map((item, idx) => (
          <Link
            ...
            onClick={handleLinkClick}
            className="block w-full px-4 py-4 mb-2 bg-white border border-gray-200 rounded-lg hover:border-brand hover:bg-brand/5 transition-all duration-150 focus:outline-none min-h-[52px]"
          >
